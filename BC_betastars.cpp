...
vec startobeta(vec const& betastar){
  ...
  // Value Pack
  beta[2] = exp(betastar[2]); // VP better than VP-
  
  // S-SYNC
  beta[3] = exp(betastar[3]); // SYN better than SYN-
  
  //Battery
  beta[4] = exp(betastar[4]); // 8-12 >= 4-8
  
  //SD Slot 
  beta[5] = -exp(betastar[5]); //SD- worse than SD+
  
  //Resolution
  beta[6] = exp(betastar[6]); // HiRes better than standard
  
  
  // Order constraints
  
  //Storage
  beta[7] = exp(betastar[7]); // 16 >= 8
  beta[8] = beta[7]+exp(betastar[8]); // 32 >= 16
  beta[9] = beta[8]+exp(betastar[9]); // 64 >= 32
  beta[10] = beta[9]+exp(betastar[10]); // 128 >= 64
  
  //Performance
  beta[11] = exp(betastar[11]); // 1.6 >= 1
  beta[12] = beta[11]+exp(betastar[12]); // 2.2 >= 1.6
  
  //Connection
  beta[13] = exp(betastar[13]); // WLAN3G >= WLAN
  beta[14] = beta[13]+exp(betastar[14]); // WLAN4G >= WLAN3G
  
  // Basic Equipment Items: Cover, Keyboard, Mouse, Pencil, 32 GB Memory Card ≥0
  beta[15] = exp(betastar[15]); // Cover (C) ≥0
  beta[16] = exp(betastar[16]); // Keyboard (K) ≥0
  beta[17] = exp(betastar[17]); // Mouse (M) ≥0
  beta[18] = exp(betastar[18]); // Pencil (P) ≥0
  beta[19] = exp(betastar[19]); // 32 GB Memory Card (32MC) ≥0
  
  // Keyboard + Pencil >= Keyboard and Pencil
  beta[20] = beta[16] + beta[18] + exp(betastar[20]); // Keyboard + Pencil (KP) ≥ Keyboard (β25) and Pencil (β27)
  
  // Keyboard + Mouse + Pencil >= Keyboard + Pencil and Mouse
  beta[21] = beta[20] + beta[17] + exp(betastar[21]); // Keyboard + Mouse + Pencil (KMP) ≥ Keyboard + Pencil (KP) and Mouse (β26)
  
  //Cashback
  beta[22] = exp(betastar[22]); // 50 >= 0
  beta[23] = beta[22]+exp(betastar[23]); // 100 >= 50
  beta[24] = beta[23]+exp(betastar[24]); // 150 >= 100
  
  // Unrestricted attributes
  beta(span(25,nvar-1)) = betastar(span(25,nvar-1)); //double check that its ranging from 25 to 37
...
mnlMetropOnceOut mnlMetropOnce_mod(vec const& y, mat const& X, vec const& oldbeta, 
  ...
  //betac(0)=oldbeta(0); // TEST TEST no budget update from initial value
