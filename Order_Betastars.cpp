...
vec startobeta(vec const& betastar){
  ...
  //Sign constraints for Tablets
  
  // Price
  beta[0] = -exp(betastar[0]);
  
  // Value Pack
  beta[1] = exp(betastar[1]); // VP better than VP-
  
  // S-SYNC
  beta[2] = exp(betastar[2]); // SYN better than SYN-
  
  //Battery
  beta[3] = exp(betastar[3]); // 8-12 >= 4-8
  
  //SD Slot 
  beta[4] = -exp(betastar[4]); //SD- worse than SD+
  
  //Resolution
  beta[5] = exp(betastar[5]); // HiRes better than standard
  
  
  // Order constraints
  
  //Storage
  beta[6] = exp(betastar[6]); // 16 >= 8
  beta[7] = beta[6]+exp(betastar[7]); // 32 >= 16
  beta[8] = beta[7]+exp(betastar[8]); // 64 >= 32
  beta[9] = beta[8]+exp(betastar[9]); // 128 >= 64
  
  //Performance
  beta[10] = exp(betastar[10]); // 1.6 >= 1
  beta[11] = beta[10]+exp(betastar[11]); // 2.2 >= 1.6
  
  //Connection
  beta[12] = exp(betastar[12]); // WLAN3G >= WLAN
  beta[13] = beta[12]+exp(betastar[13]); // WLAN4G >= WLAN3G
  
  // Basic Equipment Items: Cover, Keyboard, Mouse, Pencil, 32 GB Memory Card ≥0
  beta[14] = exp(betastar[14]); // Cover (C) ≥0
  beta[15] = exp(betastar[15]); // Keyboard (K) ≥0
  beta[16] = exp(betastar[16]); // Mouse (M) ≥0
  beta[17] = exp(betastar[17]); // Pencil (P) ≥0
  beta[18] = exp(betastar[18]); // 32 GB Memory Card (32MC) ≥0
  
  // Keyboard + Pencil >= Keyboard and Pencil
  beta[19] = beta[15] + beta[17] + exp(betastar[19]); // Keyboard + Pencil (KP) ≥ Keyboard (β25) and Pencil (β27)
  
  // Keyboard + Mouse + Pencil >= Keyboard + Pencil and Mouse
  beta[20] = beta[19] + beta[16] + exp(betastar[20]); // Keyboard + Mouse + Pencil (KMP) ≥ Keyboard + Pencil (KP) and Mouse (β26)
  
  //Cashback
  beta[21] = exp(betastar[21]); // 50 >= 0
  beta[22] = beta[21]+exp(betastar[22]); // 100 >= 50
  beta[23] = beta[22]+exp(betastar[23]); // 150 >= 100
  
  // Unrestricted attributes
  beta(span(24,nvar-1)) = betastar(span(24,nvar-1)); //double check that its ranging from 24 to 35
