rhierMnlRwMixture_SR_ord <- function (Data, Prior, Mcmc, nvar_c, flag) 
{
  ...
  out = optim(betastarinit, llmnl_mod, method = "Nelder-Mead", control = list(fnscale = -1, 
                                                                              trace = 0, reltol = 1e-8), X = Xpooled, y = ypooled)
  if(out$convergence != 0 & flag == "exact"){
  ...
  }else if(flag=="approx"|flag=="untuned"){
  ...
  for (i in 1:nlgt) {
    ...
    if (out$convergence == 0 & flag != "untuned") {
